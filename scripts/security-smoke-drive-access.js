...
function createMultipartRequest(fields, file) {
    ...
    body.push(`Content-Type: ${file.contentType}`);
    body.push('');
...
async function main() {
  ...
  const userAToken = await loginAndGetToken(USER_A_EMAIL, USER_A_PASSWORD);
  const superAdminToken = await loginAndGetToken(SUPER_ADMIN_EMAIL, SUPER_ADMIN_PASSWORD);
  
  let itemInTaskBId = null;
  await runner.run('[Setup] Super admin can upload to Task B', async () => {
    const request = createMultipartRequest(
        { task_id: taskB.id },
        { fieldName: 'file', fileName: 'test-b.txt', contentType: 'text/plain', content: 'test-content-b' }
    );
    const res = await fetch(`${API_BASE_URL}/drive/upload`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${superAdminToken}`, 'Content-Type': request.contentType },
        body: request.body,
    });
  ...
  await runner.run('User A can upload to their own unit (Task A)', async () => {
    const request = createMultipartRequest(
        { task_id: taskA.id },
        { fieldName: 'file', fileName: 'test-a.txt', contentType: 'text/plain', content: 'test-content-a' }
    );
    const res = await fetch(`${API_BASE_URL}/drive/upload`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${userAToken}`, 'Content-Type': request.contentType },
        body: request.body,
    });
    if (res.status !== 200) throw new Error(`Expected 200, got ${res.status}. Body: ${await res.text()}`);
    const json = await res.json();
  ...
  if (itemInTaskAId) {
    await runner.run('User A can get thumbnail for item in their own unit', async () => {
        ...
        if (![200, 404].includes(res.status)) throw new Error(`Expected 200 or 404 (if thumbnail not ready), got ${res.status}.`);
    });
  }
  ...
  await runner.run('User A CANNOT upload to another unit (Task B)', async () => {
    const request = createMultipartRequest(
        { task_id: taskB.id },
        { fieldName: 'file', fileName: 'test-a-fail.txt', contentType: 'text/plain', content: 'test-content-a-fail' }
    );
    const res = await fetch(`${API_BASE_URL}/drive/upload`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${userAToken}`, 'Content-Type': request.contentType },
